...
if(inverseWeights){
	 arguments = list(fun = output$distribution[1], minRad = minRad, maxRad = maxRad, 
	 w = wAnnulus,	 output$parameter1[1], output$parameter2[1] )
	 arguments = arguments[!is.na(arguments)]
	 denom = do.call(simpIntQuad, arguments)
	 arguments2 = list((minRad:maxRad) - 0, spec = output$distribution[1],  a = minRad-1, b = maxRad, 
	 output$parameter1[1], output$parameter2[1] )
	 arguments3 = list((minRad:maxRad) + 1, spec = output$distribution[1],  a = minRad, b = maxRad, 
	 output$parameter1[1], output$parameter2[1] )
	 arguments2 = arguments2[!is.na(arguments2)]
	 arguments3 = arguments3[!is.na(arguments3)]
	 numerator = wAnnulus * (do.call(ptrunc, arguments3) - do.call(ptrunc, arguments2))
	  fStarTable = cbind( minRad:maxRad, wAnnulus, numerator/denom)
	  colnames(fStarTable) = c('annulusOuter', 'inputWeight', 'prop')
	 return(list(output = output,  fStarTable = fStarTable))
	 } else {
	return(output)
	}
}  #function
